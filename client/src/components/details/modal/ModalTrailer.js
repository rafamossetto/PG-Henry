...
function ModalTrailer({ showModalTrailer, setShowModalTrailer }) {
    ...
    const handleSubmit = async (e) => {
        ...
        await swal(`${'Trailer URL'}`, `Edited succesfully`, "success", {
            buttons: false,
            timer: 2000,
        });
    ...
    const animation = useSpring({
        ...
        opacity: showModalTrailer ? 1 : 0,
        transform: showModalTrailer ? `translateY(0%)` : `translateY(-100%)`
    })
        ...
        <div>
            ...
            <StyledContainer ref={modalRef} onClick={closeModal}>
                ...
                >
                    <button
                        className="close"
                        onClick={() => setShowModalTrailer(prev => !prev)}
                    > X
                    </button>

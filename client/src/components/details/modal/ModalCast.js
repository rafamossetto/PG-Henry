...
function ModalCast({ showModalCast, setShowModalCast }) {
    ...
    const handleSubmit = async (e) => {
        ...
        await swal(`${'Cast'}`, `Edited succesfully`, "success", {
            buttons: false,
            timer: 2000,
        });
    ...
    const animation = useSpring({
        ...
        opacity: showModalCast ? 1 : 0,
        transform: showModalCast ? `translateY(0%)` : `translateY(-100%)`
    })
        ...
        <div>
            ...
            <StyledContainer ref={modalRef} onClick={closeModal}>
                ...
                >
                    <button
                        className="close"
                        onClick={() => setShowModalCast(prev => !prev)}
                    > X
                    </button>
